...
export function BreakdownShell({
  ...
  useEffect(() => {
    ...
    styleEl.setAttribute("data-breakdown-print", "");
    // A4 横で、ツールバー類は印刷しない。各営業所ページは改ページ。
    styleEl.textContent = `
      @media print {
        @page { size: A4 landscape; margin: 8mm; }
        .no-print { display: none !important; }
        .breakdown-page + .breakdown-page { page-break-before: always; }
        .breakdown-page { max-width: none !important; margin: 0 !important; padding: 4mm !important; }
      }
    `;
  ...
  function navigate(newDate: string, newBranchId: number | null) {
    ...
    router.push(`/print/breakdown?${params.toString()}`);
  }
